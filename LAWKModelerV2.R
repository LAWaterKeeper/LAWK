...
#Filter analytes to only contain those found to be most important from here: https://github.com/LAWaterKeeper/LAWK/blob/main/LAWKModelerV1.R
set.seed(1)
sampleNum <- 200
LoopNum <- 1000
ImportantVars <- c("Chloride (mg/l)","Phosphorus as P (mg/l)","smcshed","Ammonia as N (mg/l)","Nitrogen Kjeldahl (mg/l)","Sulfate (mg/l)","Copper (mg/l)","OrthoPhosphate as P (mg/l)","Hardness as CaCO3 (mg/l)")
AnalyteFreq <- AnalyteFreq[AnalyteFreq$analytename %in% ImportantVars,]
...
for(i in 1:LoopNum){
  ...
  rf1 <- suppressWarnings(tuneRF(StreamsTrain[,c(ImportantVars)],StreamsTrain$csci,stepFactor=1,plot=FALSE,doBest=TRUE))
  print(i)
...
write.table(RFEvaluationSignificant,paste("RFEvaluationSimplified",LoopNum,"Iterations",sampleNum,"Samples.txt",sep=""),quote=FALSE,sep="\t",row.names = FALSE)
...
write.table(RFImportanceTotal,paste("RFImportanceSimplified",LoopNum,"Iterations",sampleNum,"Samples.txt",sep=""),quote=FALSE,sep="\t",row.names = FALSE)
